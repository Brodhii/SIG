...
function buildPopup(properties, propertyKey) {
  ...
  // Khusus untuk titik tangkapan - HANYA 3 data ini
  if (properties['Alat_Tangk'] || properties['Jenis_Tang'] || properties['Jumlah_Tan']) {
    const parts = [];
    if (properties['Jenis_Tang']) parts.push(`<b>Jenis:</b> ${properties['Jenis_Tang']}`);
    if (properties['Alat_Tangk']) parts.push(`<b>Alat:</b> ${properties['Alat_Tangk']}`);
    if (properties['Jumlah_Tan']) parts.push(`<b>Jumlah:</b> ${properties['Jumlah_Tan']}`);
